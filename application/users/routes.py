...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Váš účet bol vytvorený! Teraz sa môžete prihlásiť', 'success')
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Bol odoslaný e-mail s pokynmi na obnovenie hesla.', 'info')
        return redirect(url_for('users.login'))
    return render_template('reset_request.html', title='Zmena hesla', form=form)
...
def reset_token(token):
    ...
    if user is None:
        flash('Toto je neplatný alebo vypršaný token', 'warning')
